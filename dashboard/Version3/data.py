...
def get_group_standings() -> pd.DataFrame:
    ...
    expected_extra = {t: 0.0 for t in teams_df["team"]}
    for _, m in unplayed.iterrows():
        expected_extra[m.team1] += m.team1_win_prob * 3 + m.draw_prob * 1
        expected_extra[m.team2] += m.team2_win_prob * 3 + m.draw_prob * 1
    ...
    for _, t in teams_df.iterrows():
        ...
        rows.append({
            ...
            "played": c["played"], "w": c["w"], "d": c["d"], "l": c["l"], "points": c["points"],
            "goals_for": c["gf"], "goals_against": c["ga"], "goal_difference": c["gf"] - c["ga"],
            "projected_points": round(c["points"] + expected_extra[t["team"]], 1),
...
@lru_cache(maxsize=1)
def get_knockout_schedule() -> pd.DataFrame:
    """Round of 32 -> Final fixtures with no teams decided yet — date/venue only."""
    return rd.load_knockout_skeleton()[["stage", "date", "kickoff_et", "venue", "city"]].copy()
