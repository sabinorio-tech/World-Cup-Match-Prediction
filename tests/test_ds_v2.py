"""
DS v2 test suite — covers outputs of 04_goals_trend.ipynb, 05_ensemble_v2.ipynb,
and src/predict_v2.py.
...
def features_v5():
    return pd.read_csv(ROOT / "data/processed/features_v5.csv", parse_dates=["date"])
...
# ---------------------------------------------------------------------------
# 1. features_v5.csv  (output of 04_goals_trend.ipynb)
# ---------------------------------------------------------------------------
...
class TestXGBV2Artifact:
    ...
    def test_artifact_keys(self, xgb_v2_artifact):
        required = {
            ...
            "result_map", "result_map_inv", "h2h_fallback",
        }
...
class TestPredictV2:
    ...
    def test_v2_differs_from_v1_for_h2h_rich_pairs(self, predict_v2_fn):
        # France vs Argentina have real H2H history — v2 should differ from v1
        sys.path.insert(0, str(ROOT))
        from src.predict import predict_match as predict_v1
        r_v1 = predict_v1("France", "Argentina")
        r_v2 = predict_v2_fn("France", "Argentina")
        assert r_v1 != r_v2, "v2 predictions should differ from v1 for H2H-rich pairs"
