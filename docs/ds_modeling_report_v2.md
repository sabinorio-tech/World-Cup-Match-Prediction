...
> v2 adds head-to-head (H2H) features to both XGBoost and Poisson, achieving the best test log loss to date.
...
Output: `data/processed/features_v5.csv` (16,610 × 19).
...
| File | Description |
|---|---|
| `models/xgb_model_v2.pkl` | XGBoost v2, 13 features, best_iter=90 |
| `models/poisson_model_v2.pkl` | Poisson v2, 6 features (home + away), with scalers |

Both artifacts include `result_map`, `result_map_inv`, and fallback values for prediction.
...
from src.predict_v2 import predict_match
...
Same interface as v1 (`predict.py`), just import from `predict_v2` instead. H2H is computed automatically from historical data.
...
Still valid — `data/processed/predictions_2026.csv` was generated by v1. For v2 predictions, use Option A dynamically.
