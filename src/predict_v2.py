...
with open(_ROOT / 'models' / 'xgb_model_v2.pkl', 'rb') as f:
    _xgb = pickle.load(f)
...
_h2h_fallback     = _xgb['h2h_fallback']

with open(_ROOT / 'models' / 'poisson_model_v2.pkl', 'rb') as f:
    _poi = pickle.load(f)
...
def _get_h2h(
    ...
    mask = (
        (
            ((_df['home_team'] == home_team) & (_df['away_team'] == away_team)) |
            ((_df['home_team'] == away_team) & (_df['away_team'] == home_team))
        ) &
        (_df['date'] < date)
    )
    recent = _df[mask].tail(n)
    if len(recent) < min_matches:
        return _h2h_fallback.copy()
...
def predict_match(
    ...
    """Predict match outcome probabilities using XGBoost v2 + Poisson v2 ensemble.

    Both models include head-to-head history features.
